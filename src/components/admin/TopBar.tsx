...
export default function TopBar({ onMenuClick }: TopBarProps) {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-white/10 bg-canvas/70 px-4 backdrop-blur-xl lg:px-6">
      ...
      <div className="flex items-center gap-3 lg:gap-4">
        <button
          ...
          aria-label="返回前台"
          className="flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm text-slate-400 hover:bg-white/5 hover:text-slate-200"
        >
          <ArrowLeft className="h-4 w-4" />
          <span className="hidden sm:inline">返回前台</span>
        </button>

        <div className="flex items-center gap-3 border-l border-white/10 pl-3 lg:pl-4">
