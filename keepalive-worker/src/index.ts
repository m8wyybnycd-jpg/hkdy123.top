  ...
  try {
    ...
    if (type === "oauth") {
      ...
      if (needsRenewal && credential.auto_renew && credential.renew_endpoint) {
        const renewResult = await renewOAuthToken(credential, secret);
    ...
    // 401/403 = auth failure; 404 = endpoint might not have /models but key is valid
...
async function renewOAuthToken(
  ...
  try {
    ...
    // Note: Re-encryption and DB update would happen here in a full implementation.
    // For now, we log the successful renewal. The actual token update would require
    // the encryptCredential function (not imported in this Worker to keep it minimal).
    // TODO: Import and use encryptCredential for full renewal flow.

    console.log(`[keepalive] OAuth token renewed for credential ${credential.id} (${credential.name})`);
...
async function healthCheckWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    const result = await healthCheck(credential, secret);
...
export default {
  async scheduled(
    ...
    console.log(`[keepalive] Cron triggered at ${new Date().toISOString()}`);

    // Fetch all credentials that are due for a health check
    // Check interval: every 30 minutes for active, every 60 for error status
    ...
    for (const credential of credentials) {
      try {
        const healthResult = await healthCheckWithRetry(credential, JWT_SECRET, 2);
  ...
  async fetch(
    ...
    // POST /trigger — manually trigger a health check (protected by secret token)
    if (url.pathname === "/trigger" && request.method === "POST") {
      ...
      // Trigger the same logic as scheduled
      await this.scheduled(
        { scheduledTime: Date.now(), cron: "manual" } as unknown as ScheduledEvent,
        env,
