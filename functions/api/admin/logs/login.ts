...
/**
 * GET /api/admin/logs/login — 登录日志列表（分页 + 筛选）。
 * GET /api/admin/logs/login?export=csv — 导出 CSV。
 *
 * 需要 log:view 权限。
 *
 * 查询参数：
 * - search:  按用户名搜索
 * - status:  按状态筛选（success/fail）
 * - method:  按登录方式筛选（email/sms）
 * - startDate: 开始日期
 * - endDate:   结束日期
 * - page:    页码
 * - pageSize: 每页条数
  ...
  try {
    ...
    if (exportCsv) {
      const result = await DB.prepare(
        `SELECT id, user_id, username, ip, user_agent, status, method, created_at
      ...
      const csvLines: string[] = [
        "ID,用户ID,用户名,IP,User-Agent,状态,方式,时间",
      ];
      ...
      return new Response(csv, {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="login_logs_${Date.now()}.csv"`,
        },
      });
    }
    ...
    const result = await DB.prepare(
      `SELECT * FROM login_logs ${whereClause}
